...
Feature: BonusBuyIn Balance and Loss-Limit
  ...
  Scenario Outline: Casino Bonus is excluded from Casino loss
    When I update the loss limit to <LossLimit> for 24 hours
    And I place multiple spins on "<GameProvider>" Casino "<GameType>" game with a stake of <Stake>
    Then the <ProviderType> error code should be <ErrorCode>
    And the <ProviderType> error message should be <ErrorMessage>

    Examples:
      | ProviderType | LossLimit | GameProvider | GameType    | Stake              | ErrorMessage                        | ErrorCode |
      | PlayTech     | $40.00    | Playtech     | fatesisters | 200.00             | Warning : Insufficent funds         | 108       |
      | PlayTech     | $40.00    | Playtech     | fatesisters | 10.00, 40.00, 3.00 | Error : 24 Hour Loss Limit Breached | 129       |
      | PlayTech     | $40.00    | Playtech     | fatesisters | 80.00              |                                     | 0         |